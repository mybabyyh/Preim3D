...
from datasets.images_dataset import ImagesDatasetCars
...
class Coach:
    ...
    def train(self):
        ...
        while self.global_step < self.opts.max_steps:
            ...
            for batch_idx, batch in enumerate(self.train_dataloader):
                ...
                if self.opts.id_lambda_multi_view > 0 and self.opts.id_lambda_multi_view > 0 and self.global_step > self.opts.multi_view_start_step:
    ...
    def generate_image(self, batch_size):
        ...
        yaw = random.uniform(-1., 1.) * math.radians(30.)
        pitch = random.uniform(-1., 1.) * math.radians(20.)
        camera_lookat_point = torch.tensor(self.net.decoder.rendering_kwargs['avg_camera_pivot'],
                                           device=self.device)
        cam2world_pose = LookAtPoseSampler.sample(math.pi / 2 + yaw, math.pi / 2 + pitch,
                                                  camera_lookat_point,
    ...
    def generate_multiview_surround_image(self, latent, poses):
        ...
        yaw = random.uniform(-1., 1.) * math.radians(20.)
        pitch = random.uniform(-1., 1.) * math.radians(5.)
        ...
        for yaw in yaws:
            for pitch in pitches:
                for i in range(batch_size):
                    ...
                    camera_lookat_point = torch.tensor(self.net.decoder.rendering_kwargs['avg_camera_pivot'],
                                                       device=self.device)
                    cam2world_pose = LookAtPoseSampler.sample(math.pi / 2 - pose[1] + yaw, math.pi / 2 - pose[0] + pitch, camera_lookat_point,
                                                              radius=self.net.decoder.rendering_kwargs[
                                                                  'avg_camera_radius'],
                                                              device=self.device)
    ...
    def generate_multiview_random_image(self, view_count, latent):
        ...
        for i in range(view_count*batch_size):
            cam2world_pose = GaussianCameraPoseSampler.sample(math.pi / 2 + yaw, math.pi / 2 + pitch, horizontal_stddev=0.25,
    ...
    def calc_loss(self, x, y, y_hat, latent, w_rec=False, latent_gt=None, multi_imgs=None):
        ...
        # multiview id loss
        if self.opts.view_count > 0 and self.opts.id_lambda_multi_view > 0 and self.global_step > self.opts.multi_view_start_step:
